...
func validateEmbeddingCount(t *testing.T, resp *schemas.BifrostEmbeddingResponse, wantCount int) {
	...
	got := len(resp.Data)
	// Some providers collapse batch into a 2D array on a single data entry.
	if got == 1 && resp.Data[0].Embedding.Embedding2DArray != nil {
		got = len(resp.Data[0].Embedding.Embedding2DArray)
	}
	if got != wantCount {
