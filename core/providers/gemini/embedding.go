...
// EmbeddingInputToContents returns the Contents slice from the input.
// Each element maps to one output embedding; parts within an element are aggregated.
func EmbeddingInputToContents(input *schemas.EmbeddingInput) ([]schemas.EmbeddingContent, error) {
	if input == nil {
		return nil, fmt.Errorf("embedding input is nil")
	}
...
func ToGeminiEmbeddingRequest(bifrostReq *schemas.BifrostEmbeddingRequest) (*GeminiBatchEmbeddingRequest, error) {
	...
	contents, err := EmbeddingInputToContents(bifrostReq.Input)
	if err != nil || len(contents) == 0 {
		return nil, fmt.Errorf("error converting embedding input to contents: %w", err)
	}
...
func ToGeminiEmbeddingResponse(bifrostResp *schemas.BifrostEmbeddingResponse) *GeminiEmbeddingResponse {
	...
	for i, embedding := range bifrostResp.Data {
		...
		geminiEmbedding := GeminiEmbedding{Values: values}
